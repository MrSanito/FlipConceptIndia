...
export default function InquireModal({ product, isOpen, onClose }) {
  ...
  const onSubmit = async (data) => {
    ...
    if (result.success) {
      ...
      alert("Failed to send email. Please try again or use direct email.");
    }
  };
  ...
  const openEmail = () => {
    ...
    const subject = `Follow up Inquiry for: ${product?.name || "General Query"}`;
    const text = `I just sent an email inquiry via the website.`;
